					 ...
					 if (strncmp(data, "getid", 5) == 0)
						{
						 ...
						 else if(strncmp(data, "setip", 5) == 0)
						{

						  	tcp_write(tpcb,"IP and PORT SET OK !\r\n",sizeof("IP and PORT SET OK !\r\n"), 1);
							 tcp_output(tpcb);
						
						}
						  
						else if(memcmp(&IDoff,data, 5)==0)
						{
  ...
  {	
    	...
    	struct ip_addr ip_addr;	
			...
			*/	 	   
			  ...
			  IP4_ADDR(&ip_addr, SeverIP[0],SeverIP[1],SeverIP[2],SeverIP[3]);
			...
			tcp_connect(wpcb, &ip_addr, SeverPORT, tcp_client_connected);
